...
var _itemsjs = require("itemsjs");
var _itemsjsDefault = parcelHelpers.interopDefault(_itemsjs);
const itemsjs = (0, _itemsjsDefault.default)((0, _dataJsonDefault.default), {
    sortings: {
        title_asc: {
            field: "title",
            order: "asc"
...
},{"itemsjs":"ELANg","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./data.json":"aLYkf"}],"ELANg":[function(require,module,exports) {
"use strict";
...
module.exports = function itemsjs(items, configuration) {
    ...
    var fulltext;
    if (configuration.native_search_enabled !== false) fulltext = new Fulltext(items, configuration);
     // index facets
...
},{"./lib":"j421d","./helpers":"01rST","./fulltext":"k3NAQ","./facets":"1aYES"}],"j421d":[function(require,module,exports) {
"use strict";
 ...
 */ module.exports.search = function(items, input, configuration, fulltext, facets) {
    ...
    if (configuration.native_search_enabled === false && (input.query || input.filter)) throw new Error('"query" and "filter" options are not working once native search is disabled');
    var search_time = 0;
    ...
    if (query_ids) filtered_indexes_bitmap = query_ids;
    if (facet_result.ids) filtered_indexes_bitmap = filtered_indexes_bitmap.new_intersection(facet_result.ids);
    if (facet_result.not_ids) filtered_indexes_bitmap = filtered_indexes_bitmap.new_difference(facet_result.not_ids);
   ...
   */ var paginationApplied = false;
    ...
    var sorting_time = 0;
    if (input.sort) filtered_items = module.exports.sorted_items(filtered_items, input.sort, configuration.sortings);
    else if (_ids) {
 ...
 */ module.exports.sorted_items = function(items, sort, sortings) {
    if (sortings && sortings[sort]) sort = sortings[sort];
    if (sort.field) return _.orderBy(items, sort.field, sort.order || "asc");
    return items;
};
/**
 ...
 */ module.exports.similar = function(items, id, options) {
    ...
    if (!options.field) throw new Error("Please define field in options");
    var field = options.field;
    var sorted_items = [];
    for(var _i = 0; _i < items.length; ++_i)if (items[_i].id !== id) {
    ...
    sorted_items = _.orderBy(sorted_items, [
        "intersection_length"
    ], [
        "desc"
    ]);
 ...
 */ module.exports.aggregation = function(items, input, configuration, fulltext, facets) {
    ...
    if (input.name && (!configuration.aggregations || !configuration.aggregations[input.name])) throw new Error('Please define aggregation "'.concat(input.name, '" in config'));
...
},{"lodash":"3qBDj","./helpers":"01rST","fastbitset":"4UKUd"}],"3qBDj":[function(require,module,exports) {
var global = arguments[3];
(function() {
...
},{}],"01rST":[function(require,module,exports) {
"use strict";
...
var filters_matrix = function filters_matrix(facets, query_filters) {
    ...
    if (!temp_facet["is_temp_copied"]) _.mapValues(temp_facet["bits_data"], function(values, key) {
   ...
   */ _.mapValues(query_filters, function(conjunction) {
        ...
        _.mapValues(conjunction, function(filter) {
            ...
            if (!temp_facet["bits_data_temp"][filter_key]) throw new Error("Panic. The key does not exist in facets lists.");
            if (conjunctive_index && temp_facet["bits_data_temp"][filter_key][filter_val]) conjunctive_index = temp_facet["bits_data_temp"][filter_key][filter_val].new_intersection(conjunctive_index);
            else if (conjunctive_index && !temp_facet["bits_data_temp"][filter_key][filter_val]) conjunctive_index = new FastBitSet([]);
            else conjunctive_index = temp_facet["bits_data_temp"][filter_key][filter_val];
        });
        union = (union || new FastBitSet([])).new_union(conjunctive_index || new FastBitSet([]));
    });
    if (union !== null) _.mapValues(temp_facet["bits_data_temp"], function(values, key) {
 ...
 */ var matrix = function matrix(facets, filters) {
   ...
   */ _.mapValues(filters, function(filter) {
    ...
    if (conjunctive_index) _.mapValues(temp_facet["bits_data_temp"], function(values, key) {
        _.mapValues(temp_facet["bits_data_temp"][key], function(facet_indexes, key2) {
    ...
    _.mapValues(temp_facet["bits_data_temp"], function(values, key) {
        _.mapValues(temp_facet["bits_data_temp"][key], function(facet_indexes, key2) {
            _.mapValues(disjunctive_indexes, function(disjunctive_index, disjunctive_key) {
                if (disjunctive_key !== key) temp_facet["bits_data_temp"][key][key2] = temp_facet["bits_data_temp"][key][key2].new_intersection(disjunctive_index);
            });
        });
    });
    return temp_facet;
...
var index = function index(items, fields) {
    ...
    _.chain(items).map(function(item) {
        fields.forEach(function(field) {
            //if (!item || !item[field]) {
            if (!item) return;
            if (!facets["data"][field]) facets["data"][field] = {};
            if (Array.isArray(item[field])) item[field].forEach(function(v) {
                if (!item[field]) return;
                if (!facets["data"][field][v]) facets["data"][field][v] = [];
...
var getBuckets = function getBuckets(data, input, aggregations) {
    ...
    return _.mapValues(data["bits_data_temp"], function(v, k) {
        ...
        var buckets = _.chain(v).toPairs().map(function(v2) {
            var filters = [];
            if (input && input.filters && input.filters[k]) filters = input.filters[k];
            var doc_count = v2[1].array().length;
            if (hide_zero_doc_count && doc_count === 0) return;
            return {
                key: v2[0],
                doc_count: doc_count,
        ...
        if (show_facet_stats) {
            ...
            _.chain(v).toPairs().forEach(function(v2) {
                if (isNaN(v2[0])) throw new Error("You cant use chars to calculate the facet_stats.");
                 // Doc_count
                if (v2[1].array().length > 0) v2[1].forEach(function(doc_count) {
...
var mergeAggregations = function mergeAggregations(aggregations, input) {
    return _.mapValues(clone(aggregations), function(val, key) {
        if (!val.field) val.field = key;
        var filters = [];
        if (input.filters && input.filters[key]) filters = input.filters[key];
        val.filters = filters;
        var not_filters = [];
        if (input.not_filters && input.not_filters[key]) not_filters = input.not_filters[key];
        if (input.exclude_filters && input.exclude_filters[key]) not_filters = input.exclude_filters[key];
...
var input_to_facet_filters = function input_to_facet_filters(input, config) {
    ...
    _.mapValues(input.filters, function(values, key) {
        if (values && values.length) {
            if (config[key].conjunction !== false) _.mapValues(values, function(values2) {
                filters.push([
                    key,
                    values2
                ]);
    ...
    _.mapValues(input.not_filters, function(values, key) {
        if (values && values.length) _.mapValues(values, function(values2) {
            filters.push([
                key,
                "-",
                values2
...
var parse_boolean_query_temp = function parse_boolean_query_temp(query) {
    ...
    return _.map(result, function(v1) {
        if (Array.isArray(v1)) return _.map(v1, function(v2) {
            if (Array.isArray(v2)) return _.map(v2, function(v3) {
                return v3;
            });
            else return v2.split(":");
        });
        else return v1.split(":");
    });
};
...
},{"lodash":"3qBDj","fastbitset":"4UKUd","boolean-parser":"dlwe1"}],"4UKUd":[function(require,module,exports) {
...
function FastBitSet(iterable) {
    ...
    if (iterable) {
        ...
        } else for(let i = 0; i < iterable.length; i++)this.add(iterable[i]);
    }
}
...
FastBitSet.prototype.size = function() {
    ...
    const c = this.words.length;
    const w = this.words;
    for(let i = 0; i < c; i++)answer += this.hammingWeight(w[i]);
    return answer;
};
...
FastBitSet.prototype.intersection = function(otherbitmap) {
    ...
    const c = this.words.length;
    for(k = newcount; k < c; ++k)this.words[k] = 0;
    return this;
};
...
FastBitSet.prototype.intersection_size = function(otherbitmap) {
    const newcount = Math.min(this.words.length, otherbitmap.words.length);
    let answer = 0;
    for(let k = 0; k < newcount; ++k)answer += this.hammingWeight(this.words[k] & otherbitmap.words[k]);
    return answer;
};
// Computes the intersection between this bitset and another one,
...
FastBitSet.prototype.new_intersection = function(otherbitmap) {
    ...
    for(; k < count; ++k)answer.words[k] = this.words[k] & otherbitmap.words[k];
    return answer;
};
// Computes the intersection between this bitset and another one,
...
FastBitSet.prototype.equals = function(otherbitmap) {
    ...
    if (this.words.length < otherbitmap.words.length) {
        ...
        for(let k1 = this.words.length; k1 < c; ++k1){
            if (otherbitmap.words[k1] != 0) return false;
        }
    } else if (otherbitmap.words.length < this.words.length) {
        const c1 = this.words.length;
        for(let k2 = otherbitmap.words.length; k2 < c1; ++k2){
            if (this.words[k2] != 0) return false;
        }
    }
    return true;
};
...
FastBitSet.prototype.difference2 = function(otherbitmap) {
    ...
    for(; k < mincount; ++k)otherbitmap.words[k] = this.words[k] & ~otherbitmap.words[k];
    // remaining words are all part of difference
    for(k = this.words.length - 1; k >= mincount; --k)otherbitmap.words[k] = this.words[k];
...
FastBitSet.prototype.difference_size = function(otherbitmap) {
    ...
    let answer = 0;
    let k = 0;
    for(; k < newcount; ++k)answer += this.hammingWeight(this.words[k] & ~otherbitmap.words[k]);
    const c = this.words.length;
    for(; k < c; ++k)answer += this.hammingWeight(this.words[k]);
    return answer;
};
...
FastBitSet.prototype.change = function(otherbitmap) {
    ...
    for(; k < mincount; ++k)this.words[k] ^= otherbitmap.words[k];
    // remaining words are all part of change
    for(k = otherbitmap.words.length - 1; k >= mincount; --k)this.words[k] = otherbitmap.words[k];
    return this;
};
...
FastBitSet.prototype.new_change = function(otherbitmap) {
    ...
    const c = this.words.length;
    for(k = mcount; k < c; ++k)answer.words[k] = this.words[k];
    const c2 = otherbitmap.words.length;
    for(k = mcount; k < c2; ++k)answer.words[k] = otherbitmap.words[k];
    return answer;
};
...
FastBitSet.prototype.change_size = function(otherbitmap) {
    ...
    let answer = 0;
    let k = 0;
    for(; k < mincount; ++k)answer += this.hammingWeight(this.words[k] ^ otherbitmap.words[k]);
    const longer = this.words.length > otherbitmap.words.length ? this : otherbitmap;
    const c = longer.words.length;
    for(; k < c; ++k)answer += this.hammingWeight(longer.words[k]);
    return answer;
};
...
FastBitSet.prototype.union = function(otherbitmap) {
    ...
    if (this.words.length < otherbitmap.words.length) {
        ...
        const c = otherbitmap.words.length;
        for(let k1 = mcount; k1 < c; ++k1)this.words[k1] = otherbitmap.words[k1];
    }
    return this;
};
...
FastBitSet.prototype.union_size = function(otherbitmap) {
    const mcount = Math.min(this.words.length, otherbitmap.words.length);
    let answer = 0;
    for(let k = 0; k < mcount; ++k)answer += this.hammingWeight(this.words[k] | otherbitmap.words[k]);
    if (this.words.length < otherbitmap.words.length) {
        const c = otherbitmap.words.length;
        for(let k1 = this.words.length; k1 < c; ++k1)answer += this.hammingWeight(otherbitmap.words[k1] | 0);
    } else {
        const c1 = this.words.length;
        for(let k2 = otherbitmap.words.length; k2 < c1; ++k2)answer += this.hammingWeight(this.words[k2] | 0);
    }
    return answer;
};
///////////////
...
},{}],"dlwe1":[function(require,module,exports) {
// Boolean-parser.js
// -----------------
// License: MIT
...
function _arraysAreEqual(arrA, arrB) {
    if (!Array.isArray(arrA) || !Array.isArray(arrB)) throw new TypeError("both parameters have to be an array");
...
function parseBooleanQuery(searchPhrase) {
    ...
    var orPath = ors.map(function(andQuery) {
        ...
        for(var i = 0; i < ands.length; i++)// If the string contains brackets, parse it recursively, and add it to
        // `nestedPaths`.
        if (containsBrackets(ands[i])) nestedPaths.push(parseBooleanQuery(ands[i]));
        else andPath.push(ands[i]);
...
function orsAndMerge(ors) {
    ...
    for(var i = 0; i < ors.length; i++)result = orAndOrMerge(result, ors[i]);
    return result;
}
...
function deduplicateOr(orPath, orderMatters) {
    var path = orderMatters ? orPath : orPath.map(function(item) {
        return item.sort();
    });
    return path.reduce(function(memo, current) {
        for(var i = 0; i < memo.length; i++){
            if (_arraysAreEqual(memo[i], current)) return memo;
        }
        memo.push(current);
        return memo;
    }, []);
...
function mergeOrs(ors) {
    var result = ors[0];
    for(var i = 1; i < ors.length; i++)result = result.concat(ors[i]);
    return result;
}
    ...
    if (phrase.charAt(0) === "(") {
        ...
        for(var i = 0; i < phrase.length; i++){
            ...
            if (counter === 0) {
                ...
                if (i !== phrase.length - 1) return phrase;
                else return phrase.substring(1, phrase.length - 1);
            }
        }
    }
    return phrase;
...
function splitRoot(splitTerm, phrase) {
    ...
    var result = [];
    var tempNested = [];
    for(var i = 0; i < termSplit.length; i++)// If we are dealing with a split in a nested query,
...
},{}],"k3NAQ":[function(require,module,exports) {
"use strict";
var _ = require("lodash");
 ...
 */ var Fulltext = function Fulltext(items, config) {
     ...
     */ if (config.removeStopWordFilter) this.pipeline.remove(lunr.stopWordFilter);
    });
    var i = 1;
...
Fulltext.prototype = {
    ...
    search: function search(query, filter) {
        var _this3 = this;
        if (!query && !filter) return this.items ? this.items.map(function(v) {
            return v._id;
        }) : [];
        var items;
        if (query) items = _.map(this.idx.search(query), function(val) {
            var item = _this3.store[val.ref];
            return item;
        });
        if (filter instanceof Function) items = (items || this.items).filter(filter);
...
},{"lodash":"3qBDj","lunr":"f973C"}],"f973C":[function(require,module,exports) {
(function() {
    /**
 ...
 */ lunr.tokenizer.registerFunction = function(fn, label) {
        if (label in this.registeredFunctions) lunr.utils.warn("Overwriting existing tokenizer: " + label);
        fn.label = label;
 ...
 */ lunr.Pipeline.load = function(serialised) {
        ...
        serialised.forEach(function(fnName) {
            ...
            else throw new Error("Cannot load un-registered function: " + fnName);
        });
        return pipeline;
    };
    /**
 ...
 */ lunr.Vector.prototype.dot = function(otherVector) {
        ...
        while(node && otherNode){
            if (node.idx < otherNode.idx) node = node.next;
            else if (node.idx > otherNode.idx) otherNode = otherNode.next;
            else {
 ...
 */ lunr.SortedSet.prototype.union = function(otherSet) {
        ...
        if (this.length >= otherSet.length) longSet = this, shortSet = otherSet;
        else longSet = otherSet, shortSet = this;
 ...
 */ lunr.Index.load = function(serialisedData) {
        if (serialisedData.version !== lunr.version) lunr.utils.warn("version mismatch: current " + lunr.version + " importing " + serialisedData.version);
        var idx = new this;
 ...
 */ lunr.Index.prototype.search = function(query) {
        var queryTokens = this.pipeline.run(this.tokenizerFn(query)), queryVector = new lunr.Vector, documentSets = [], fieldBoosts = this._fields.reduce(function(memo, f) {
            return memo + f.boost;
        }, 0);
 ...
 */ lunr.TokenStore.prototype.add = function(token, doc, root) {
        ...
        if (rest.length === 0) {
            ...
            this.length += 1;
            return;
        } else return this.add(rest, doc, root[key]);
    };
    /**
...
},{}],"1aYES":[function(require,module,exports) {
"use strict";
var _ = require("lodash");
var helpers = require("./helpers");
var FastBitSet = require("fastbitset");
 ...
 */ var Facets = function Facets(items, configuration) {
    ...
    this.ids_map = {};
    if (items) items.forEach(function(v) {
        var custom_id_field = configuration.custom_id_field || "id";
        if (v[custom_id_field] && v._id) _this.ids_map[v[custom_id_field]] = v._id;
    });
...
Facets.prototype = {
    ...
    bits_ids: function bits_ids(ids) {
        if (ids) return new FastBitSet(ids);
   ...
   */ search: function search(input, data) {
        ...
        _.mapValues(temp_facet["bits_data_temp"], function(values, key) {
            _.mapValues(temp_facet["bits_data_temp"][key], function(facet_indexes, key2) {
                if (data.query_ids) temp_facet["bits_data_temp"][key][key2] = data.query_ids.new_intersection(temp_facet["bits_data_temp"][key][key2]);
                if (data.test) temp_facet["data"][key][key2] = temp_facet["bits_data_temp"][key][key2].array();
            });
        });
        /**
     ...
     */ if (input.filters_query) temp_facet.ids = helpers.filters_ids(temp_facet["bits_data_temp"]);
        else temp_facet.ids = helpers.facets_ids(temp_facet["bits_data_temp"], input.filters);
        return temp_facet;
    }
};
module.exports = Facets;

},{"lodash":"3qBDj","./helpers":"01rST","fastbitset":"4UKUd"}],"gkKU3":[function(require,module,exports) {
...
module.exports = JSON.parse('[{"id":"1","href":"https://website.com","tags":["tag 1","tag 2"],"title":"New tag 1 and tag 2 title","description":"Some description here"},{"id":"2","href":"https://website.com","tags":["tag 4"],"title":"New tag 4 title","description":"Some description here"},{"id":"3","href":"https://website.com","tags":["tag 3"],"title":"New tag 3 title","description":"Some description here"},{"id":"4","href":"https://website.com","tags":["tag 2","tag 5"],"title":"New tag 2 and tag 5 title","description":"Some description here"}]');

},{}]},["7Aums","bNKaB"], "bNKaB", "parcelRequire7d1c")
